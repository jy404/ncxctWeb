...
$('.reg_num').blur(function(){
  if ((/^\d+$/).test($(".reg_num").val())){
    $('.num_hint').html("✔").css("color","green");
    num_Boolean = true;
  }else {
    $('.num_hint').html("×").css("color","red");
    num_Boolean = false;
  }
});
//纯文本验证
$('.reg_wen').blur(function(){
  if ((/[\w\W]+/).test($(".reg_wen").val())){
    $('.wen_hint').html("✔").css("color","green");
    wen_Boolean = true;
  }else {
    $('.wen_hint').html("×").css("color","red");
    wen_Boolean = false;
  }
});
